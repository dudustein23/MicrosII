...
/* USER CODE BEGIN PV */
uint8_t atual = 0;
uint8_t frequencias[] = {261,227,293,311,329,349,370,392,415};
ESTADO_COMANDO comando_sinal = NENHUM;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if (GPIO_Pin == controller1.GPIO_Pin){
		...
		if (controller1.estado == AO_PRESSIONAR){
			...
			case 311:
				...
				break;
			case 329:
				HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase5, (uint16_t) sizeof(frase5));
				break;
			case 349:
				HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase6, (uint16_t) sizeof(frase6));
				break;
			case 370:
				HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase7, (uint16_t) sizeof(frase7));
				break;
			case 392:
				HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase8, (uint16_t) sizeof(frase8));
				break;
			case 415:
				HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase9, (uint16_t) sizeof(frase9));
				break;
			case 261:
			default:
	...
	if (GPIO_Pin == controller2.GPIO_Pin){
		...
		if (controller2.estado == AO_PRESSIONAR){
			HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase10, (uint16_t) sizeof(frase10));
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef * huart)
{
	...
	if ((comando_numero != 0) || (comando_sinal != NENHUM)) {
		gerencia_estados();
		comando_numero = 0;
		comando_sinal = NENHUM;
	}
	HAL_UART_Receive_IT(&huart2, &comando, TCMD);
}

void incrementa_decrementa(){
	switch(comando)
	{
	case '+':
		comando_sinal = INCREMENTA;
		break;
	case '-':
		comando_sinal = DECREMENTA;
		break;
	default:
		comando_sinal = NENHUM;
		break;
...
void gerencia_estados()
{
	...
	if (comando_numero != 0) {
		atual = comando_numero - 1;
		HAL_UART_Transmit_IT(&huart2, (uint8_t *) frase1, (uint16_t) sizeof(frase1));
	}
	setaFrequenciaTim(&htim10, frequencias[atual]);
}
